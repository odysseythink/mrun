...
func LevenshteinCalc(str1, str2 []rune, maxCost, insCost, subCost, delCost int) (dist, prefixLen, suffixLen int) {
...
func LevenshteinDistance(str1, str2 string, p *LevenshteinParams) int {
	if p == nil {
		p = defaultLevenshteinParams
	}
	dist, _, _ := LevenshteinCalc([]rune(str1), []rune(str2), p.maxCost, p.insCost, p.subCost, p.delCost)
	return dist
}
...
func LevenshteinSimilarity(str1, str2 string, p *LevenshteinParams) float64 {
	return LevenshteinMatch(str1, str2, p.Clone().BonusThreshold(1.1)) // guaranteed no bonus
}
...
// If a non-zero MinScore value is provided in the parameters, scores lower than it
// will be returned as 0.
func LevenshteinMatch(str1, str2 string, p *LevenshteinParams) float64 {
	...
	dist, pl, _ := LevenshteinCalc(s1, s2, max, p.insCost, p.subCost, p.delCost)
	if max > 0 && dist > max {
		return 0
	}
