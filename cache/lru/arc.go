...
type ARCCache[K comparable, V any] struct {
	...
	t1 LRUCacher[K, V]        // T1 is the LRU for recently accessed items
	b1 LRUCacher[K, struct{}] // B1 is the LRU for evictions from t1

	t2 LRUCacher[K, V]        // T2 is the LRU for frequently accessed items
	b2 LRUCacher[K, struct{}] // B2 is the LRU for evictions from t2
...
func NewARC[K comparable, V any](size int) (*ARCCache[K, V], error) {
	// Create the sub LRUs
	b1, err := NewSimpleLRU[K, struct{}](size, nil)
	if err != nil {
		return nil, err
	}
	b2, err := NewSimpleLRU[K, struct{}](size, nil)
	if err != nil {
		return nil, err
	}
	t1, err := NewSimpleLRU[K, V](size, nil)
	if err != nil {
		return nil, err
	}
	t2, err := NewSimpleLRU[K, V](size, nil)
	if err != nil {
		return nil, err
	}
