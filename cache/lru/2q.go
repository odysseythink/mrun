...
type TwoQueueCache[K comparable, V any] struct {
	...
	recent      LRUCacher[K, V]
	frequent    LRUCacher[K, V]
	recentEvict LRUCacher[K, struct{}]
	lock        sync.RWMutex
}
...
func New2QParams[K comparable, V any](size int, recentRatio, ghostRatio float64) (*TwoQueueCache[K, V], error) {
	...
	// Allocate the LRUs
	recent, err := NewSimpleLRU[K, V](size, nil)
	if err != nil {
		return nil, err
	}
	frequent, err := NewSimpleLRU[K, V](size, nil)
	if err != nil {
		return nil, err
	}
	recentEvict, err := NewSimpleLRU[K, struct{}](evictSize, nil)
	if err != nil {
		return nil, err
	}
