...
func TestSimpleLRU_GetOldest_RemoveOldest(t *testing.T) {
	l, err := NewSimpleLRU[int, int](128, nil)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	for i := 0; i < 256; i++ {
...
func TestSimpleLRU_Add(t *testing.T) {
	evictCounter := 0
	onEvicted := func(k int, v int) {
		evictCounter++
	}
...
func TestSimpleLRU_Contains(t *testing.T) {
	l, err := NewSimpleLRU[int, int](2, nil)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
...
func TestSimpleLRU_Peek(t *testing.T) {
	l, err := NewSimpleLRU[int, int](2, nil)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
...
func TestSimpleLRU_Resize(t *testing.T) {
	onEvictCounter := 0
	onEvicted := func(k int, v int) {
		onEvictCounter++
	}
...
func (c *SimpleLRU[K, V]) wantKeys(t *testing.T, want []K) {
	t.Helper()
	got := c.Keys()
...
func TestSimpleLRUCache_EvictionSameKey(t *testing.T) {
	var evictedKeys []int

	cache, _ := NewSimpleLRU(
		2,
