...
func TestMapPut(t *testing.T) {
	...
	m.Put(2, "b")
	m.Put(1, "a") //overwrite
	tmp, err := json.Marshal(m.m)
	if err != nil {
		fmt.Printf("json.Marshal(%#v) failed:%v\n", m.m, err)
	} else {
		fmt.Printf("-----------tmp=%v\n", string(tmp))
		tmpmap := New[int, string]()
		err = json.Unmarshal(tmp, &tmpmap.m)
		if err != nil {
			fmt.Printf("json.Marshal(%#v) failed:%v\n", m.m, err)
		} else {
			fmt.Printf("-----------tmpmap=%v\n", tmpmap.m)
...
func benchmarkGet[V any](b *testing.B, m *Map[int, V], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			m.Get(n)
		}
...
func benchmarkPut[V any](b *testing.B, m *Map[int, V], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			var val V
			m.Put(n, val)
