...
type Tree struct {
	Root       *Node                 // Root node
	Comparator containers.Comparator // Key comparator
...
func NewWith(order int, comparator containers.Comparator) *Tree {
	if order < 3 {
...
func NewWithIntComparator(order int) *Tree {
	return NewWith(order, containers.IntComparator)
}
...
func NewWithStringComparator(order int) *Tree {
	return NewWith(order, containers.StringComparator)
}
