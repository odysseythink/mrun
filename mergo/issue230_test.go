...
func withOverrideEmptyValue(enable bool) func(*Config) {
	if enable {
		return WithOverwriteWithEmptyValue
	}
...
func TestMergeMapWithOverride(t *testing.T) {
	...
	for _, data := range testDataM {
		err := Merge(&data.M2, data.M1, withOverrideEmptyValue(data.WithOverrideEmptyValue))
		if err != nil {
