...
func New(name string, dataPath string, maxBytesPerFile int64,
	...
	if err != nil && !os.IsNotExist(err) {
		mlog.Errorf("DISKQUEUE(%s) failed to retrieveMetaData - %s", d.name, err)
	}
...
func (d *diskQueue) exit(deleted bool) error {
	...
	if deleted {
		mlog.Infof("DISKQUEUE(%s): deleting", d.name)
	} else {
		mlog.Infof("DISKQUEUE(%s): closing", d.name)
	}
...
func (d *diskQueue) Empty() error {
	...
	mlog.Infof("DISKQUEUE(%s): emptying", d.name)
...
func (d *diskQueue) deleteAllFiles() error {
	...
	if innerErr != nil && !os.IsNotExist(innerErr) {
		mlog.Errorf("DISKQUEUE(%s) failed to remove metadata file - %s", d.name, innerErr)
		return innerErr
	}
...
func (d *diskQueue) skipToNextRWFile() error {
	...
	for i := d.readFileNum; i <= d.writeFileNum; i++ {
		...
		if innerErr != nil && !os.IsNotExist(innerErr) {
			mlog.Errorf("DISKQUEUE(%s) failed to remove data file - %s", d.name, innerErr)
			err = innerErr
		}
	}
...
func (d *diskQueue) readOne() ([]byte, error) {
	...
	if d.readFile == nil {
		...
		mlog.Infof("DISKQUEUE(%s): readOne() opened %s", d.name, curFileName)
...
func (d *diskQueue) writeOne(data []byte) error {
	...
	if d.writePos > 0 && d.writePos+totalBytes > d.maxBytesPerFile {
		...
		err = d.sync()
		if err != nil {
			mlog.Errorf("DISKQUEUE(%s) failed to sync - %s", d.name, err)
		}
	...
	if d.writeFile == nil {
		...
		mlog.Infof("DISKQUEUE(%s): writeOne() opened %s", d.name, curFileName)
...
func (d *diskQueue) retrieveMetaData() error {
	...
	if d.writePos < fileSize {
		mlog.Warningf("DISKQUEUE(%s) %s metadata writePos %d < file size of %d, skipping to new file",
...
func (d *diskQueue) checkTailCorruption(depth int64) {
	...
	if depth != 0 {
		if depth < 0 {
			mlog.Errorf("DISKQUEUE(%s) negative depth at tail (%d), metadata corruption, resetting 0...",
				d.name, depth)
		} else if depth > 0 {
			mlog.Errorf("DISKQUEUE(%s) positive depth at tail (%d), data loss, resetting 0...",
				d.name, depth)
		}
		// force set depth 0
		d.depth = 0
	...
	if d.readFileNum != d.writeFileNum || d.readPos != d.writePos {
		if d.readFileNum > d.writeFileNum {
			mlog.Errorf("DISKQUEUE(%s) readFileNum > writeFileNum (%d > %d), corruption, skipping to next writeFileNum and resetting 0...",
				d.name, d.readFileNum, d.writeFileNum)
		}

		if d.readPos > d.writePos {
			mlog.Errorf("DISKQUEUE(%s) readPos > writePos (%d > %d), corruption, skipping to next writeFileNum and resetting 0...",
...
func (d *diskQueue) moveForward() {
	...
	if oldReadFileNum != d.nextReadFileNum {
		...
		err := os.Remove(fn)
		if err != nil {
			mlog.Errorf("DISKQUEUE(%s) failed to Remove(%s) - %s", d.name, fn, err)
		}
	}
...
func (d *diskQueue) handleReadError() {
	...
	mlog.Warningf("DISKQUEUE(%s) jump to next file and saving bad file as %s",
		d.name, badRenameFn)

	err := os.Rename(badFn, badRenameFn)
	if err != nil {
		mlog.Errorf("DISKQUEUE(%s) failed to rename bad diskqueue file %s to %s",
	...
	for {
		...
		if (d.readFileNum < d.writeFileNum) || (d.readPos < d.writePos) {
			if d.nextReadPos == d.readPos {
				...
				if err != nil {
					mlog.Errorf("DISKQUEUE(%s) reading at %d of %s - %s",
...
exit:
	mlog.Infof("DISKQUEUE(%s): closing ... ioLoop", d.name)
	syncTicker.Stop()
	d.exitSyncChan <- 1
}
